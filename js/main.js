...
startBtn.addEventListener('click', function(){
    time = prompt('Please enter your date of birth using  YYYY-MM-DD formate', '');
...
countBtn.addEventListener('click', function(){
    ...
    if (appData.moneyPerDay < 100) {
        levelValue.textContent = 'Minimum amount needed';
        
    } else if(appData.moneyPerDay > 100 && appData.moneyPerDay < 2000){
        levelValue.textContent = 'Average amount needed';
        
    } else if (appData.moneyPerDay > 2000){
        levelValue.textContent = 'High amount needed';
        
    } else {
        levelValue.textContent = 'occured error';
        
    } 
    } else {
        dayBudgetValue.textContent = 'error occurred';
    }
...
let appData = {
    ...
    checkSavings: function(){
        if (appData.savings == true) {
            let save = +prompt('How much is the saving amount'), 
            percent = +prompt('What is the ARP?');
    
            appData.monthIncome = save/100/12*percent;
    
            alert('income from the saving: ' + appData.monthIncome );
